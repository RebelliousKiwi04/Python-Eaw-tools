...
class ModItem():
    def __init__(self, name,mod_dir, image='tempimage2.png', mod_id=None, steammod=False,allMods=[]):
        ...
        self.data_dict['ARGUMENTS'] = ''
        self.data_dict['PARENT_MOD'] = None
        self.normal_build = 'G'
        self.debug_build = 'I'
    ...
    def run(self):
        if self.steam:
            subprocess.Popen([f"StarWars{self.build_id}.exe",f"STEAMMOD={self.mod_id}"])
        else:
            subprocess.Popen([f"StarWars{self.build_id}.exe","MODPATH=Mods\\"+self.name])
