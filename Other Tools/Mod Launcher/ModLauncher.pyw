...
import os, requests, sys,subprocess,getpass
from winreg import *
...
#     r.write(exeIconByte)
with open('tempimage2.png', 'wb') as r:
    r.write(eawIconByte)
...
class ModItem():
    def __init__(self, name,mod_dir, image='tempimage2.png', mod_id=None, steammod=False,allMods=[]):
        self.steam = steammod
    ...
    def run(self):
        if self.steam:
            subprocess.Popen([f"StarWars{self.build_id}.exe",self.mod_launch])
        else:
            subprocess.Popen([f"StarWars{self.build_id}.exe",self.mod_launch])
    def configure_mod(self):
        ...
        if self.data_dict['PARENT_MOD'] != "Do Not Run As Submod":
            self.parentMod.setCurrentIndex(self.data_dict['PARENT_INDEX']+1)
    ...
    def save_configuration(self):
        parent = self.parentMod.currentText()
        ...
        if parent == "Do Not Run As Submod":
            ...
            self.data_dict['PARENT_MOD'] = self.mods[mod_index]
            self.data_dict['PARENT_INDEX'] = mod_index
            self.launch_options = self.mods[mod_index].mod_launch + ' ' + self.mod_launch
            print(self.launch_options)
            print(self.mods[mod_index].name)
...
class ModLauncherWindow:
    def __init__(self):
        ...
        self.exeImageLocation = os.getcwd()+'\\tempimage2.png'
        font = QFont()
        font.setPointSize(10)
